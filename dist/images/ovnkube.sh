...
#OVN_ROUTED_UDN_ISOLATION - enable pod network isolation between advertised UDN networks.
ovn_routed_udn_isolation=${OVN_ROUTED_UDN_ISOLATION:=true}
...
ovn-master() {
  ...
  if [[ ${ovn_routed_udn_isolation} == "true" ]]; then
	  routed_udn_isolation_flag="--routed-udn-isolation"
  fi
