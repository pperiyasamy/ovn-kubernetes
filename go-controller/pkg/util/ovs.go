...
func GetGeneveTunnels() ([]TunnelInfo, error) {
	var geneveTunnels []TunnelInfo
	stdout, stderr, err := RunOVSVsctl("--format=json", "--columns=name,options",
	...
	// sample output
	// {"data":[["ovn-203886-0",["map",[["csum","true"],["key","flow"],["local_ip","10.89.0.3"],["remote_ip","10.89.0.4"],["remote_name","203886dd-d6e1-4b48-bdd9-0a6bbfc6acdc"]]]],
	//          ["ovn-4e658c-0",["map",[["csum","true"],["key","flow"],["local_ip","10.89.0.3"],["remote_ip","10.89.0.2"],["remote_name","4e658c65-a43f-4bb9-a36f-723ef1d96a66"]]]]],
	//  "headings":["name","options"]}
	data := result["data"].([]interface{})
	for _, item := range data {
		entry := item.([]interface{})
		name := entry[0].(string)
		options := entry[1].([]interface{})
		if len(options) >= 2 && options[0] == "map" {
			optionMap := make(map[string]string)
			for _, option := range options[1].([]interface{}) {
				entry := option.([]interface{})
				key := entry[0].(string)
				value := entry[1].(string)
				optionMap[key] = value
			}
			geneveTunnels = append(geneveTunnels,
				TunnelInfo{Name: name, LocalIP: optionMap["local_ip"], RemoteIP: optionMap["remote_ip"], RemoteName: optionMap["remote_name"]})
		}
	}
	return geneveTunnels, nil
}
