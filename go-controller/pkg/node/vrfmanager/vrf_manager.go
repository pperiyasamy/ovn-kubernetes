...
func (c *Controller) runInternal(stopChan <-chan struct{}, doneWg *sync.WaitGroup, subscribe subscribeFn) {
	...
	go func() {
		...
		for {
			...
			case newVrf := <-c.addVrfCh:
				if err = c.addVrf(&newVrf); err != nil {
					klog.Errorf("Vrf manager: failed to add vrf (%v): %v", newVrf, err)
				}
			case delVrf := <-c.delVrfCh:
				if err = c.delVrf(&delVrf); err != nil {
...
// AddVrf submits a request to add a Vrf
func (c *Controller) AddVrf(name string, table uint32, interfaces sets.Set[string], routes []netlink.Route) {
	c.addVrfCh <- vrf{name, table, interfaces, routes, false}
}

// DeleteVrf submits a request to delete a Vrf
func (c *Controller) DeleteVrf(name string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	vrf, ok := c.vrfs[name]
	if !ok {
