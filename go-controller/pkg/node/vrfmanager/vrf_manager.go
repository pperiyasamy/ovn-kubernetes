...
func (vrfm *Controller) Run(stopCh <-chan struct{}, doneWg *sync.WaitGroup) {
	...
	vrfm.runInternal(stopCh, doneWg, subscribe, unsubscribe)
}
...
func (vrfm *Controller) runInternal(stopChan <-chan struct{}, doneWg *sync.WaitGroup,
	subscribe subscribeFn, unsubscribe unsubscribeFn) {
	doneWg.Add(1)
	go func() {
		defer doneWg.Done()
		linkSyncTimer := time.NewTicker(120 * time.Second)
		...
		for {
			...
			case linkUpdateEvent, ok := <-linkUpdateCh:
				...
				link := linkUpdateEvent.Link.Attrs().Name
				klog.Infof("Vrf manager: link %s update received", link)
				err = vrfm.syncLink(linkUpdateEvent.Link)
				if err != nil {
					klog.Errorf("Vrf manager: Error syncing link %s update event: %v", link, err)
				}
