...
func getMetricValue(f *framework.Framework, podName, podNamespace, metricName string) string {
	...
	cmd := exec.Command("prom2json", metricURL)
	output, err := cmd.CombinedOutput()
	if err != nil {
		framework.Failf("prom2json failed: %v, output: %s", err, string(output))
	}
	...
	framework.Failf("metric %s not found in prom2json output", metricName)
	return ""
}
